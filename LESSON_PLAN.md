...
| 0:54–1:04 | **Group into families (embeddings + clustering)** | Embed each snag as a point in space, then **k-means** sorts *all* of them into 5 families on a 2-D map — no labels. Honest ~0.45 match to the real systems (Mechanical pops out cleanly; others blend); read the top words to name each family. The unsupervised companion to the duplicate finder. |
