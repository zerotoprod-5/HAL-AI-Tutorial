...
# Plain Markdown (an arrow line) so it renders identically in Colab with no
# colour and no HTML <div>.
# ---------------------------------------------------------------------------
PIPELINE = (
    "**The whole pipeline, end to end:**\n\n"
    "> 🎙️ **Speak** &rarr; 📝 **Transcribe (ASR)** &rarr; 🗂️ **Categorize**\n\n"
    "- **Speak** &mdash; a technician says a quick report out loud: "
    "*\"There is an oil leak near the hydraulic pump.\"*\n"
    "- **Transcribe** &mdash; the computer listens and turns the audio into plain **text** "
    "(speech-to-text / ASR).\n"
    "- **Categorize** &mdash; the **same text analytics** from the text sessions sorts that "
    "text into a category &rarr; **Hydraulic**."
)

# A tiny "neither magic nor nonsense" recap, in plain Markdown, tying the whole
# day's four sessions together. One idea, three kinds of data.
SERIES_TABLE = (
...
# ---- Banner + how to run -------------------------------------------------
md(banner("Speech Analytics &middot; Session 4 of 4", "Speech Analytics: From Spoken Report to Decision",
...
md(bigidea(
   ...
   "First it <b>transcribes</b> the speech into text. Then it runs the <b>same text analytics</b> you already saw in the text sessions. "
...
md(note("This first cell is the <b>only</b> heavy setup in the whole day, and it takes about a minute "
...
md(did("The computer <b>listened to sound and produced text</b> &mdash; and it is (nearly) word-for-word the sentence we spoke. "
       ...
       "ordinary text, which means we can feed it straight into the same text analytics from the text sessions.")),
...
md("Now we rebuild the text-analytics idea from the text sessions, self-contained right here. We give the computer a small set of past spoken-style "
...
md(did("One <code>.fit()</code> call &mdash; the same word you saw in the earlier sessions &mdash; and the model has learned which words "
...
code(
   ...
   "    'There is an oil leak near the hydraulic pump and the pressure is dropping.'\n"
   "\n"
   "# Feed the transcribed text into the trained classifier.\n"
...
md(recap("What we learned in this notebook", [
   ...
   "Once speech becomes text, it is just text &mdash; so the <b>exact text-analytics method</b> classifies it, no new ideas needed.",
...
# ---- CLOSING: the day's finale -- wrap the whole workshop ----------------
md(section("Wrapping up the day", None)),
md(bigidea(
...
md(SERIES_TABLE),
md(nextup(
   "<b>That is the whole day.</b> Across four sessions one idea kept returning: <b>turn it into numbers, learn the pattern, "
   "judge the new case</b> &mdash; and always check the result honestly. You classified written maintenance notes "
   "(<b>urgent vs routine</b>), found the <b>recurring themes</b> hiding in free text, taught a model to <b>hear a failing "
   "machine</b> from its sound alone, and finally turned the <b>spoken word into text</b> and measured how often it mishears. "
   "Same recipe, four very different signals &mdash; neither magic nor nonsense, and now firmly in your hands.")),
]

build(cells, os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "notebooks", "04_speech_analytics.ipynb"),
